...
import images from "../../data/gallery"; // <- imports the array from gallery.js
...
export default function GallerySection() {
  ...
  useEffect(() => {
    // determine mobile by tailwind sm breakpoint (640px)
  ...
  useEffect(() => {
    ...
    // start autoplay after small timeout so initial render settles
    ...
    <section className="py-12 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="mt-4 text-2xl sm:text-3xl font-extrabold text-[hsl(220_14%_16%)]">
            Campus & Life — A Visual Tour
          </h2>

          <p className="mt-2 text-[hsl(220_9%_46%)] max-w-xl mx-auto">
            Pictures from classrooms, labs, hospital, herbal garden and student life.
          </p>
        </div>
        ...
        <div className="hidden sm:block">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {images.map((src, idx) => (
              <div key={idx} className="overflow-hidden rounded-lg h-48 md:h-56 bg-gray-100">
                <img
                  src={src}
                  alt={`gallery-${idx}`}
        ...
        <div className="sm:hidden mt-2">
          <div ref={sliderRef} className="w-full overflow-hidden">
            <div
              ...
              style={{ scrollSnapType: "x mandatory", WebkitOverflowScrolling: "touch", overflowX: "auto" }}
            >
              {images.map((src, idx) => (
                <div
                  key={idx}
                ...
                >
                  <div className="relative h-56 bg-gray-100">
                    <img src={src} alt={`gallery-mobile-${idx}`} className="w-full h-full object-cover rounded-lg" />
                    <div className="absolute top-3 left-3 rounded-md p-2 bg-white/60 backdrop-blur-sm">
                      <svg className="w-5 h-5 text-[hsl(152_59%_30%)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                        <path d="M3 7v10a2 2 0 0 0 2 2h14" />
                        <path d="M21 7V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v2" />
                      </svg>
                    </div>
            ...
            {/* small pager dots (static) */}
            <div className="flex items-center justify-center gap-2 mt-3">
              {images.map((_, i) => (
                <span key={i} className="w-2 h-2 bg-[hsl(220_9%_46%)]/30 rounded-full" />
              ))}
            </div>
          </div>
        </div>

        {/* See more button - centered below images */}
        <div className="mt-6 flex justify-center">
          <Link
            to="/gallery"
            className="inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium shadow-sm
                       bg-[hsl(152_59%_30%)] text-white hover:bg-[hsl(152_59%_30%)]/90 transition"
